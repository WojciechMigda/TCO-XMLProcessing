 ...
 *
 */
filename_w_contents_t xform_record(std::pair<std::string, tag_record_t> const & kv)
{
    nlohmann::json j;
    ...
    filename_w_contents_t rv = {kv.first, j.dump()};

    return rv;
}


Either<std::string, std::vector<filename_w_contents_t>> jsonize(tags_tree_t && tags)
{
    ...
    try
    {
        std::transform(tags.cbegin(), tags.cend(), std::back_inserter(rv), xform_record);
    }
